...
def generate_with_cache(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    for num_block in range(num_blocks):
        ...
        while (x[:, current_block_start:current_block_end] == mask_id).sum()>0:
            ...
            x0, transfer_index = get_transfer_index_opt(logits, mask_index, x, block_length, num_transfer_tokens[:, i], temperature, 
...
def generate_with_prefixcache_update(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    for num_block in range(num_blocks):
        ...
        while (x[:, current_block_start:current_block_end] == mask_id).sum()>0:
            ...
            if cfg_scale > 0.:
                ...
                past_key_values, logits = block_cache_api(model, x, past_key_values, )
                if cache_update_iter is not None and i%cache_update_iter == 0:
                    cache_update_step.append(nfe)
    ...
    # transfer_index(logits, mask_index) two variables should have same dimension[current_block_start: current_block_end]
...
@torch.no_grad()
def generate_cache_in(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
                mask_id=126336, log_flops=False, threshold=None, cache_update_iter=None, eos_early_stop=False, minimal_topk=1, **kwargs):
...
def get_transfer_index_opt(logits, mask_index, x, block_end, num_transfer_tokens, temperature, remasking='low_confidence', threshold=None, minimal_topk=1):
