...
@ torch.no_grad()
def generate_fast(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    for num_block in range(num_blocks):
        ...
        while True:
            ...
            x0, transfer_index = get_transfer_index_cur(logits_block, temperature, remasking, mask_index, x_block, num_transfer_tokens[:, i] if threshold is None else None, mask_id, threshold)
...
def generate(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    for num_block in range(num_blocks):
        ...
        while True:
            ...
            mask_index[:, prompt.shape[1] + (num_block + 1) * block_length:] = 0
            if factor is None:
                x0, transfer_index = get_transfer_index_cur(logits, temperature, remasking, mask_index, x, num_transfer_tokens[:, i] if threshold is None else None, threshold)
            else:
...
def get_transfer_index(logits, temperature, remasking, mask_index, x, num_transfer_tokens, mask_id, threshold=None):
    logits_with_noise = add_gumbel_noise(logits, temperature=temperature)
    x0 = torch.argmax(logits_with_noise, dim=-1) # b, l
...
@ torch.no_grad()
def get_transfer_index_hierarchy_remask (logits, temperature, remasking, mask_index, x, num_transfer_tokens,  
