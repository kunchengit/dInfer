...
class FusedLLaDAMoESparseMoeBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.num_experts = config.num_experts
        self.top_k = config.num_experts_per_tok
        ...
        self.w1 = nn.Parameter(
            torch.empty(self.num_experts, 2 * config.expert_intermediate_size, config.hidden_size)
        )
...
class LLaDAMoEDecoderLayer(nn.Module):
    def __init__(self, config: LLaDAConfig, layer_idx: int):
        ...
        self.mlp = FusedLLaDAMoESparseMoeBlock(config) if self.mlp_type == 'moe' else LLaDAMoEMLP(config, 'dense')
...
class FusedLLaDAMoEModelLM(LLaDAMoEPreTrainedModel):
