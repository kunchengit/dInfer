...
from dinfer.model import BailingMoeV2ForCausalLM
...
def benchmark_gen(rank, model, tokenizer, prompt, gen_len, block_len, threshold, cache, num_test_iter=1, use_sw=False, have_warmup=True, cont_weight=0.3):
    ...
    if cache == 'prefix' or cache == 'dual':
        ...
        cache_factory=None
    print('cache_factory', cache_factory)
    if cont_weight>0:
        if use_sw:
        ...
        if use_sw:
            ...
            dllm = BlockWiseDiffusionLLM(model, decoder, BlockIteratorFactory(), cache_factory=cache_factory, early_stop=True, use_shift=True)
    print(dllm)
    # warm up
    if have_warmup:
        for i in range(2):
    ...
    if rank==0:
        print(f'Iter: {num_test_iter}, Forward: {total_forward}, cache updates: {total_cache_updates}, Time: {stop-start}, FPS: {total_forward/(stop-start)}, TPS: {num_tokens/(stop-start)}')
...
def main(world_size, rank, gpu_id, args):
    ...
    with set_current_vllm_config(VllmConfig(parallel_config = parallel_config)):
        ...
        # model = AutoModel.from_pretrained(args.model_name, trust_remote_code=True, low_cpu_mem_usage=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='/mnt/dllm/luobin/dllm-mini-cpt-base-10T/')
    # parser.add_argument('--model_name', type=str, default='/mnt/dllm/luobin/dllm-flash-ckpt/500B')
